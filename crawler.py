...
class Crawler:
    ...
    def __crawl(self, current_url):
        ...
        request = Request(
            current_url, headers={"User-Agent": config.crawler_user_agent}
        )
        ...
        if self.images:
            ...
            for image_link in list(set(images)):
                ...
                # Append domain if not present
                    ...
                    image_link = (
                        f'{self.domain.strip("/")}{image_link.replace("./", "/")}'
                    )
        ...
        if date:
            lastmod = (
                "<lastmod>" + date.strftime("%Y-%m-%dT%H:%M:%S+00:00") + "</lastmod>"
            )
        ...
        url_string = (
            "<url><loc>"
            + self.htmlspecialchars(final_url)
            + "</loc>"
            + lastmod
            + image_list
            + "</url>"
    ...
    def write_index_and_sitemap_files(self):
        ...
        for i in range(0, num_sitemap_files):
            ...
            sitemap_filename = (
                sitemap_index_filename + "-" + str(i) + sitemap_index_extension
            )
        ...
        for i, sitemap_filename in enumerate(sitemap_filenames):
            self.write_subset_of_urls_to_sitemap(
                sitemap_filename, i * self.MAX_URLS_PER_SITEMAP
            )
    ...
    def write_sitemap_index(self, sitemap_filenames):
        ...
        for sitemap_filename in sitemap_filenames:
            sitemap_url = urlunsplit(
                [self.scheme, self.target_domain, sitemap_filename, "", ""]
            )
            print(
